...
def fig_bimodal():
    ...
    ax.plot(cs, amm, "o-", color="#b2182b", label="AMM market (bimodal)")
    ax.plot(cs, smooth, "s-", color="#2166ac", label="networked-contagion model (smooth)")
    ax.axhline(0.1376, ls="--", color="k", alpha=0.6, label="empirical target (0.14)")
    ax.set_xlabel("shock / contagion intensity (×)")
    ax.set_ylabel("peak contagion magnitude")
    ax.set_title("Why we replace the AMM: its depeg is bimodal,\nso it cannot be tuned to the empirical 14% target")
    ax.legend(fontsize=8); ax.grid(alpha=0.3)
    fig.tight_layout(); fig.savefig(OUT / "fig_bimodal.png", dpi=200); plt.close(fig)
...
def fig_two_agent_paths():
    ...
    cfgs = [("no agents", 0.0, 0.0, "#888888"),
            ("adaptive redeemer", 0.03, 0.0, "#b2182b"),
            ("arbitrageur", 0.0, 0.5, "#1a9850"),
            ("both", 0.03, 0.5, "#2166ac")]
    ...
    ax.set_xlabel("time step (≈5 min each)"); ax.set_ylabel("mean victim |depeg|")
    ax.set_title("Strategic agents: redeemer amplifies, arbitrageur damps\n(causal conclusions hold in every case)")
    ax.legend(fontsize=8); ax.grid(alpha=0.3)
    fig.tight_layout(); fig.savefig(OUT / "fig_two_agent_paths.png", dpi=200); plt.close(fig)
...
def fig_welfare():
    ...
    im = ax.imshow(df.values, cmap="OrRd", aspect="auto", vmin=0, vmax=float(df.values.max()))
    ...
    ax.set_title("Welfare matrix: who bears the loss under each policy\n(protect-BUSD = no intervention; protect-USDC saves everyone)")
    fig.tight_layout(); fig.savefig(OUT / "fig_welfare.png", dpi=200); plt.close(fig)
...
def fig_rl_alloc():
    ...
    colors = ["#1a9850" if n in ("USDC", "DAI") else ("#b2182b" if n == "BUSD" else "#888888") for n in names]
    ...
    ax.set_title("A PPO regulator, given only the network, learns to protect\nthe causal venues (USDC/DAI) and ignore the spurious hub (BUSD)")
    ax.set_ylim(0, 1.15); ax.grid(axis="y", alpha=0.3)
    fig.tight_layout(); fig.savefig(OUT / "fig_rl_alloc.png", dpi=200); plt.close(fig)
