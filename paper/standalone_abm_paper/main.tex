...
ablation study in machine learning. For the learned-regulator experiment we use Proximal Policy
Optimization~\cite{schulman2017ppo}, a standard policy-gradient method. We connect a graph
...
edge $W_{ij}$ is \emph{directed}, meaning a depeg in coin $j$ pushes coin $i$ down in proportion to
$W_{ij}$. We first estimate $W$ from the real minute-level data by asking which coin's moves
...
read off the documented balance sheet. Both reproduce all four moments and yield the same causal
verdict.}
\label{tab:params}
\small
\begin{tabular}{lrr}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/11_balance_sheet_gate.png}
\caption{The paper in one figure. Each panel is the calibrated contagion network
during the USDC/SVB crisis, with every venue coloured by its peak depeg from the
welfare matrix (Table~\ref{tab:interventions}). \emph{Left}, with no intervention
the shock at the origin USDC propagates to its collateral holder DAI ($-13.8\%$)
and mildly to USDP. \emph{Middle}, shielding BUSD, the predictor's \#1
correlational hub, leaves every loss \emph{identical}, a $0\%$ reduction, because
no venue holds BUSD as reserves. \emph{Right}, shielding the causal origin USDC
zeroes the entire network, a $100\%$ reduction. The same one-venue budget buys
nothing on the correlational hub and everything on the causal origin.}
\label{fig:gate}
\end{figure*}
...
The budget comparison is the headline (Figure~\ref{fig:interventions}, shown as a side-by-side
cascade in Figure~\ref{fig:gate}). A regulator who can backstop
a single venue and chooses by the correlational hub ranking protects BUSD and achieves $0\%$
contagion reduction. Choosing by the causal ranking protects USDC and achieves $100\%$. The
network panels in Figure~\ref{fig:gate} make the gap visceral, since shielding BUSD leaves every
venue's depeg unchanged while shielding USDC zeroes the whole network. The one check that
separates the two outcomes is whether any stablecoin holds the protected venue as documented reserves.
...
welfare matrix (Figure~\ref{fig:welfare}) reinforces this. Protecting BUSD leaves every victim's loss
...
budget to USDC and DAI and nothing to BUSD, achieving 94\% contagion reduction. The fact that even a
